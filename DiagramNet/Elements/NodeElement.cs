	...
	public abstract class NodeElement : BaseElement {
		...
		public override void Invalidate() {
			...
			for (int i = connects.Length - 1; i >= 0; i--) {
				...
				ConnectorElement aConnector = connects[i];

				for (int ii = aConnector.Links.Count - 1; ii >= 0; ii--) {
					BaseElement? thisLinks = aConnector.Links[ii];
					thisLinks?.Invalidate();
				}
			}
		}
