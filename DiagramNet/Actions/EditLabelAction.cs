...
namespace DiagramNet.Actions
{
	...
	internal class EditLabelAction
	{
		...
		{
			if (el is not ILabelElement) return;
		...
		public void EndEdit()
		{
			...
			if (labelTextBox is not null)	labelTextBox.KeyPress -= new KeyPressEventHandler(LabelTextBox_KeyPress);
			ILabelController? lblCtrl = siteLabelElement.GetLabelController();
			if (labelElement is not null) { 
				labelElement.Size = MeasureTextSize();
				if (labelTextBox is not null) {
		...
		{
			Rectangle tbBox = new(tb.Location, tb.Size);
		...
		private Size MeasureTextSize()
		{
			...
			if (labelTextBox != null && labelElement != null) { 
		...
		{
			if (labelTextBox is null || labelTextBox.Text.Length == 0) return;
